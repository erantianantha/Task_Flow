...
const cardBaseUrl = `${API_BASE_URL}/card`;
...
export const getCard = async (cardId, listId, boardId, dispatch) => {
	...
	try {
		...
		submitCall = submitCall.then(() =>
			axios.get(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId).then((res) => {
				response = res;
			})
		);
		await submitCall;
...
export const titleUpdate = async (cardId, listId, boardId, title, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId, { title: title })
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const descriptionUpdate = async (cardId, listId, boardId, description, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId, { description: description })
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const comment = async (cardId, listId, boardId, text, userName, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios
				.post(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/add-comment', {
					text: text,
				})
				.then((res) => {
					response = res;
...
export const commentUpdate = async (cardId, listId, boardId, text, commentId, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + commentId, {
				text: text,
			})
		);
		await submitCall;
...
export const commentDelete = async (cardId, listId, boardId, commentId, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.delete(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + commentId)
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const memberAdd = async (cardId, listId, boardId, memberId, memberName, memberColor, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.post(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/add-member', {
				memberId: memberId,
			})
		);
		await submitCall;
...
export const memberDelete = async (cardId, listId, boardId, memberId, memberName, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.delete(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + memberId + '/delete-member')
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const labelCreate = async (cardId, listId, boardId, text, color, backColor, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios
				.post(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/create-label', {
					text,
					color,
					backColor,
				})
...
export const labelUpdate = async (cardId, listId, boardId, labelId, label, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + labelId + '/update-label', label)
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const labelDelete = async (cardId, listId, boardId, labelId, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.delete(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + labelId + '/delete-label')
		);
		await submitCall;
	} catch (error) {
		dispatch(
...
export const labelUpdateSelection = async (cardId, listId, boardId, labelId, selected, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(
				cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + labelId + '/update-label-selection',
				{ selected: selected }
			)
		);
		await submitCall;
...
export const checklistCreate = async (cardId, listId, boardId, title, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios
				.post(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/create-checklist', {
					title,
				})
				.then((res) => {
					response = res;
...
export const checklistDelete = async (cardId, listId, boardId, checklistId, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.delete(
				cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + checklistId + '/delete-checklist'
			)
		);
		await submitCall;
	} catch (error) {
...
export const checklistItemAdd = async (cardId, listId, boardId, checklistId, text, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios
				.post(
					cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + checklistId + '/add-checklist-item',
					{
						text,
					}
				)
...
export const startDueDatesUpdate = async (cardId, listId, boardId, startDate, dueDate, dueTime, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/update-dates', {
				startDate,
				dueDate,
				dueTime,
			})
...
export const dateCompletedUpdate = async (cardId, listId, boardId, completed, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/update-date-completed', {
				completed,
			})
		);
		await submitCall;
...
export const attachmentAdd = async (cardId, listId, boardId, link, name, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios
				.post(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/add-attachment', {
					link: link,
					name: name,
				})
				.then((res) => {
...
export const attachmentDelete = async (cardId, listId, boardId, attachmentId, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.delete(
				cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + attachmentId + '/delete-attachment'
			)
		);
		await submitCall;
	} catch (error) {
...
export const attachmentUpdate = async (cardId, listId, boardId, attachmentId, link, name, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(
				cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/' + attachmentId + '/update-attachment',
				{ link: link, name: name }
			)
		);
		await submitCall;
...
export const coverUpdate = async (cardId, listId, boardId, color, isSizeOne, dispatch) => {
	try {
		...
		submitCall = submitCall.then(() =>
			axios.put(cardBaseUrl + '/' + boardId + '/' + listId + '/' + cardId + '/update-cover', {
				color: color,
				isSizeOne: isSizeOne,
			})
		);
